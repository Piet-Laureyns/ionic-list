...
let HomePage = class HomePage {
    constructor(modalController, alertController, itemService, toastController, popoverController) {
    ...
    searchItems(event) {
        if (event.detail) { // Check if something is typed in the search input
            ...
            if (input === '') { // If search field is empty => show entire list
                ...
                const matchingItems = this.itemService.getItemsMatchingSearchString(input);
...
let ItemService = class ItemService {
    ...
    getItemsMatchingSearchString(searchString) {
        return this.items.filter(item => item.title.toLowerCase().includes(searchString));
    }
};
